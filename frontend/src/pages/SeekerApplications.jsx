...
export default function SeekerApplications({ navProps, user, onNavigate }) {
  ...
  const counts = {
    ...
    ...Object.keys(STATUS_CONFIG).reduce(
      (acc, k) => ({ ...acc, [k]: applications.filter((a) => a.status === k).length }),
      {}
    ),
  };
      ...
      <div style={S.page}>
        ...
        <div style={S.summaryGrid}>
          {Object.entries(STATUS_CONFIG).map(([status, cfg]) => (
            <div
              key={status}
              style={{ ...S.summaryCard, borderColor: cfg.border, background: cfg.bg }}
            >
        ...
        <div style={S.filterRow}>
          {["ALL", ...Object.keys(STATUS_CONFIG)].map((f) => (
            ...
            >
              ...
              {counts[f] > 0 && <span style={S.countBadge}>{counts[f]}</span>}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div style={S.emptyState}>
            ...
            <div style={{ fontWeight: 700, fontSize: "1.1rem", marginBottom: "0.5rem" }}>
              No applications{" "}
              {filter !== "ALL" ? `with status "${STATUS_CONFIG[filter]?.label}"` : "yet"}
            </div>
            <div style={{ color: "#8888aa", fontSize: "0.9rem" }}>
              Browse jobs and start applying to track your progress here.
            </div>
            <button
              style={{ ...S.btnPrimary, marginTop: "1.5rem" }}
          ...
          <div style={S.appList}>
            {filtered.map((app, i) => {
              ...
              return (
                <div
                  key={app.id || i}
                  style={{ ...S.appCard, animation: `slideUp 0.3s ease ${i * 0.05}s both` }}
                >
